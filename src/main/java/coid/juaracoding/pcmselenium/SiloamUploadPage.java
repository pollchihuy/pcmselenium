...
//    public static String strNoBPJS;
...
//        String baseUrl = "https://dev.ptdika.com/siloam/panel/login/";
        ...
        /** Input Valid Username Priviledge User */
        fieldUsername.sendKeys("K6400001");
        delay(intDelay);
        /** Input Valid Password Priviledge User */
        fieldPassword.sendKeys("a");
        btnLogin.click();

        /** masuk menu homepage */
        delay(intDelay);
        /** Klik Link Menu Input */
        linkMenuInput.click();

        /** Skenario masuk tab input data Untuk Input Form */
        delay(intDelay);
        DataGenerator dataGenerator = new DataGenerator();
        Random r = new Random();
        mapToCompare.put("nama",dataGenerator.dataNamaLengkap());
//        String nama =  dataGenerator.dataNamaLengkap();
        fieldNama.sendKeys(mapToCompare.get("nama").toString());
        delay(intDelay);
        /** Notes : untuk pencarian nantinya
         * Step Input Valid Nomor BPJS
         * */
        String strNoBPJS = String.valueOf(r.nextLong(1000000000000L,9999999999999L));
        fieldNomorBpjs.sendKeys(strNoBPJS);

        /**
         * Step Input Valid Nomor KTP
         */
        delay(intDelay);
        fieldNoKtp.sendKeys(String.valueOf(r.nextLong(1000000000000000L,9999999999999999L)));
        delay(intDelay);

        /** Step Input valid Alamat */
        fieldAlamat.sendKeys(dataGenerator.dataAlamat());
        delay(intDelay);

        /** Step Pilih Kota KTP */
        /** sub step -> klik komponen DDL Kota KTP */
        ddlKotaKTP.click();
        delay(intDelay);
        /** sub step -> Input Ke TextField DDL Kota KTP */
        txtBoxFaskesAwal.sendKeys("KOTA JAKARTA SELATAN"+Keys.RETURN);
        delay(intDelay);

        /** Step Input Valid Faskes Awal */
        fieldFaskesAwal.sendKeys("Klinik Pondok Pinang");
        delay(intDelay);

        /** Step Pilih Faskes Tujuan */
        /** sub step -> klik komponen DDL Faskes Tujuan */
        ddlFaskesTujuan.click();
        delay(intDelay);
        /** sub step -> Input Ke TextField DDL Faskes Tujuan */
        ...
        /** Skenario masuk ke form upload document */
...
//        formUploadDocument.click();
        ...
        /** Step Upload Dokumen Faskes Tujuan */
        /** sub step - Klik Tombol Upload Faskes Tujuan */
        delay(1);
        uploadFotoFaskesTujuan.click();
        /** sub step - Upload File Faskes Tujuan */
        delay(intDelay);
        file.sendKeys("\\data\\foto-faskes-tujuan-1.png");

        /** sub step - Klik Tombol Save Faskes Tujuan*/
        delay(intDelay);
        btnSave.click();

        /** sub step - Klik Tombol OK Alert Faskes Tujuan*/
        delay(intDelay);
        btnOK.click();
...
//                .until(ExpectedConditions.elementToBeClickable(uploadFotoAskesAwal)).click();
        /** Step Upload Dokumen Faskes Awal */
        /** sub step - Klik Tombol Upload Faskes Awal */
        delay(2);
        uploadFotoAskesAwal.click();

//        file.sendKeys(System.getProperty("user.dir")+"\\data\\foto-faskes-awal-1.png");
        /** sub step - Upload File Faskes Awal */
        delay(intDelay);
        file.sendKeys("\\data\\foto-faskes-awal-1.png");

        /** sub step - Klik Tombol Save Faskes Awal */
        delay(intDelay);
        btnSave.click();

        /** sub step - Klik Tombol OK Alert Faskes Awal*/
        delay(intDelay);
        btnOK.click();

        /** Step Upload Dokumen TTD Digital */
        /** sub step - Klik Tombol Upload TTD Digital */
        delay(intDelay);
        uploadTTDDigital.click();

        /** sub step - Upload File TTD Digital */
        delay(intDelay);
        file.sendKeys(System.getProperty("user.dir")+"\\data\\ttd-digital-1.png");

        /** sub step - Klik Tombol Save TTD Digital */
        delay(intDelay);
        btnSave.click();

        /** sub step - Klik Tombol OK Alert TTD Digital*/
        delay(intDelay);
        btnOK.click();

        /** Step Konfirmasi Seluruh Dokumen Upload */
        delay(intDelay);
...
//        WebElement namaPasTTD = driver.findElement(By.xpath("//input[@id='name']"));
//        WebElement noBpjsPasTTD = driver.findElement(By.xpath("//input[@id='no_bpjs']"));
//        WebElement noKTPPasTTD = driver.findElement(By.xpath("//input[@id='no_ktp']"));
//        WebElement alamatPasTTD = driver.findElement(By.xpath("//textarea[@id='address']"));
//        boolean allIsValid = (
//                (mapToCompare.get("nama").toString().equals(namaPasTTD.getText())) &&
//                (mapToCompare.get("noBPJS").toString().equals(noBpjsPasTTD.getText())) &&
//                        (mapToCompare.get("noKTP").toString().equals(noKTPPasTTD.getText())) &&
//                        (mapToCompare.get("alamat").toString().equals(alamatPasTTD.getText()))
//        );
//        Assert.assertTrue(allIsValid);

        /** Scope Review TTD Digital */
        /** step Scroll sampai ke bawah */
        delay(intDelay);
        ((JavascriptExecutor)driver).executeScript("window.scrollBy(0,document.body.scrollHeight)");

        /** step klik tombol submit */
        delay(intDelay);
        btnSubmit.click();

        /** step tombol ok alert upload ttd digital */
        delay(intDelay);
        driver.switchTo().alert().accept();

        //kalau mau validasi lakukan ini
//        WebElement notifSuccess = driver.findElement(By.xpath("//div[@role='alert']"));
//        Assert.assertEquals(notifSuccess.getText(),"TTD Digital Berhasil");

        /** Scope Logout */
        /** Step Klik Icon Profile */
        delay(intDelay);
        iconProfile.click();

        /** Step Klik Btn Logout */
        delay(intDelay);
        btnLogOut.click();

        /** Skenario Login Positif */
        /** Step - Input Valid Username Priviledge Admin */
        fieldUsername.sendKeys("admindika");
        delay(intDelay);
        /** Step - Input Valid Password Priviledge Admin */
        fieldPassword.sendKeys("d1k4@passw0rd");
        delay(intDelay);
        /** Step - Klik Tombol Login */
        btnLogin.click();

        /** Step - Klik Menu ViewAndReport Priviledge Admin */
        delay(intDelay);
        viewAndReport.click();

        /** Step - Input DatePicker Tanggal Awal */
        //2024-06-11
        delay(intDelay);
//        dateBefore.sendKeys(new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
        dateBefore.sendKeys("2024-06-11");
//        dateBefore.sendKeys("2024-05-28");
        /** Step - Input DatePicker Tanggal Akhir */
        delay(intDelay);
